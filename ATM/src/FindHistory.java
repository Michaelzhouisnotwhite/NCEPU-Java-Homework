...
class FindHistory implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		String accountHistory = null;
		Date date = null;
		String dateHistory;
		if (e.getSource() == search) {
			ATMDriver ATMFind = new ATMDriver();
			String sql = "select tradingaccord,Date from tradingaccording where Sname='" + Id + "'";
			try {
				java.sql.Statement st = ATMFind.conn.createStatement();
				ResultSet rs = st.executeQuery(sql);
				while (rs.next()) {
					accountHistory = rs.getString("tradingaccord");
					date = rs.getDate("Date");
					dateHistory = date.toString();
					inqure.append(dateHistory + "  " + " : " + accountHistory + "\n");
				}
